var express = require('express');
var router = express.Router();
var session = require('../session');

router.get('/', async function(req, res) {
    const user = session.auth(req).user;
    if (!user) return res.status(403).send('Доступ запрещён');

    try {
        const orders = await req.db.any(`
            SELECT 
                o.id, o.label, o.amount, o.id_status,
                s.label AS status_label, 
                c.label AS client_label
            FROM orders o
            LEFT JOIN order_statuses s ON o.id_status = s.id
            LEFT JOIN clients c ON o.id_client = c.id
            ORDER BY o.id DESC
        `);
        
        const clients = await req.db.any('SELECT id, label FROM clients');
        
        res.render('orders/index', {
            orders: orders,
            clients: clients,
            user: user,
            can_create_order: user.id_role === 1 || user.id_role === 3, // admin или employee
            can_change_status: user.id_role === 1 || user.id_role === 2  // admin или manager
        });
        
    } catch(err) {
        console.error('Ошибка загрузки заказов:', err);
        res.status(500).send('Ошибка загрузки заказов');
    }
});

router.get('/create', async function(req, res) {
    const user = session.auth(req).user;
    
    // Только admin и employee могут создавать заказы
    if (!user || (user.id_role !== 1 && user.id_role !== 3)) {
        return res.status(403).send('Доступ запрещён');
    }

    try {
        const clients = await req.db.any('SELECT id, label FROM clients');
        res.render('orders/create', { 
            user: user, 
            clients: clients 
        });
    } catch(err) {
        console.error('Ошибка получения клиентов:', err);
        res.status(500).send('Ошибка получения клиентов');
    }
});

router.post('/create', async function(req, res) {
    const user = session.auth(req).user;
    
    // Только admin и employee могут создавать заказы
    if (!user || (user.id_role !== 1 && user.id_role !== 3)) {
        return res.json({ success: false, msg: 'Доступ запрещён' });
    }

    const { label, id_client, amount } = req.body;

    // Валидация
    if(!label || !id_client || !amount) {
        return res.json({ success: false, msg: 'Заполните все поля' });
    }

    const amountNum = parseFloat(amount);
    if (isNaN(amountNum) || amountNum <= 0) {
        return res.json({ success: false, msg: 'Некорректная сумма' });
    }

    try {
        // Создаем заказ
        await req.db.none(
            'INSERT INTO orders(label, id_client, amount) VALUES($1, $2, $3)',
            [label, parseInt(id_client), amountNum]
        );
        
        res.json({ success: true });
        
    } catch(err) {
        console.error('Ошибка создания заказа:', err);
        res.json({ success: false, msg: 'Ошибка создания заказа: ' + err.message });
    }
});

router.get('/:id', async function(req, res) {
    const id = parseInt(req.params.id);
    
    if (isNaN(id)) {
        return res.status(400).send("Неверный ID");
    }

    try {
        const order = await req.db.oneOrNone(`
            SELECT 
                o.id, o.label, o.amount, o.id_status,
                s.label AS status_label,
                c.label AS client_label
            FROM orders o
            LEFT JOIN order_statuses s ON o.id_status = s.id
            LEFT JOIN clients c ON o.id_client = c.id
            WHERE o.id = $1
        `, [id]);

        if (!order) {
            return res.status(404).send("Заказ не найден");
        }

        res.render('orders/show', { 
            order: order,
            user: session.auth(req).user
        });

    } catch (err) {
        console.error('Ошибка загрузки заказа:', err);
        res.status(500).send("Ошибка сервера");
    }
});

router.post('/:id/status', async function(req, res) {
    const user = session.auth(req).user;
    
    // Только admin и manager могут менять статусы
    if (!user || (user.id_role !== 1 && user.id_role !== 2)) {
        return res.status(403).send('Доступ запрещён');
    }

    const id = parseInt(req.params.id);
    const { id_status } = req.body;

    // Валидация статуса
    if (!id_status || ![10, 20, 30].includes(parseInt(id_status))) {
        return res.status(400).send('Некорректный статус');
    }

    try {
        await req.db.none(
            'UPDATE orders SET id_status = $1 WHERE id = $2', 
            [parseInt(id_status), id]
        );
        
        res.redirect(`/orders/${id}`);
        
    } catch(err) {
        console.error('Ошибка изменения статуса:', err);
        res.status(500).send('Ошибка изменения статуса');
    }
});

module.exports = router;